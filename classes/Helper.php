...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "otp_{$_args['post_type']}", 'otp' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "otp_{$_args['post_type']}", $posts, 'otp', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'otp' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'otp_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /otp-shield directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with otp-shield_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'otp_template_overwrite_dir', get_stylesheet_directory() . '/otp-shield/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = OTP_SHIELD_DIR . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'otp-shield' );
		}
	}
}